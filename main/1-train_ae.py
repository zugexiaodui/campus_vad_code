...
if __name__ == '__main__':
    ...
    model = SceneFrameAE(inp_frm=args.snippet_inp, tgt_frm=args.snippet_tgt, bgd_encoder=bgd_encoder, lam_cvae=0.)
    ...
    scalar_writer = ScalarWriter(('lr', 'img_mse', 'img_l1', 'loss_tot'), _tbxs_witer, (start_epoch - 1) * len(train_loader), start_epoch)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        for step, (batch_snp, batch_bgd, batch_lbl) in enumerate(train_loader):
            ...
            out_snp = model(inp_snp, inp_bgd)

            img_mse: torch.Tensor = criterion_mse(out_snp, tgt_snp)
            img_l1: torch.Tensor = criterion_l1(out_snp, tgt_snp)
            loss_tot: torch.Tensor = img_mse + args.lam_l1 * img_l1

            optimizer.zero_grad()
            loss_tot.backward()
            optimizer.step()

            _scalar_dict = OrderedDict(lr=lr_sch.get_last_lr()[0], img_mse=img_mse.item(), img_l1=img_l1.item(), loss_tot=loss_tot.item())
